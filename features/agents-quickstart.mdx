...
description: 'Sign up for ChatterMate and go live from the terminal — built for AI agents, CI, and automation'
---
...
# 1. Create an account (community signup) — you're logged in automatically
chattermate signup \
  --name "Acme Inc" \
  --domain acme.com \
...
- Python 3.10+ (`pipx install chattermate-cli` or `uvx --from chattermate-cli chattermate` also work)
...
Grab the agent's **widget ID** from the [dashboard](https://app.chattermate.chat) (or the agent
API), then drop this snippet on any page:
...
| Step | Command |
| --- | --- |
| Install | `pip install chattermate-cli` |
...
| Knowledge | `chattermate knowledge add-url --website … [--agent-id …]` |
